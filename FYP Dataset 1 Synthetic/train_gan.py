...
for epoch in range(epochs):
    for i, batch in enumerate(dataloader):
        ...
        gen_loss = criterion(fake_output, real_labels)  # The generator wants the discriminator to believe the fake data is real
        gen_loss.backward()
        gen_optimizer.step()

        # Print progress
        if i % 100 == 0:
            print(f"Epoch [{epoch + 1}/{epochs}], Batch [{i + 1}/{len(dataloader)}], D Loss: {disc_loss.item():.4f}, G Loss: {gen_loss.item():.4f}")

# Generate twice the amount of data
num_samples = 4958  # Desired number of samples (twice the original dataset)
latent_vectors = torch.randn(num_samples, latent_dim)
with torch.no_grad():
    generated_data = generator(latent_vectors)

# Save generated data to CSV
generated_data_df = pd.DataFrame(generated_data.numpy())
generated_data_df.to_csv('synthetic_data.csv', index=False)

print("Generated data saved to 'synthetic_data.csv'.")
